...
export const expenseCategories: any = org.table(
  "expense_categories",
  {
    ...
    parentCategoryId: uuid("parent_category_id").references((): any => expenseCategories.id),
    ...
    // Approval Settings
    approvalThreshold: numeric("approval_threshold", { precision: 15, scale: 2 }),
  ...
  (table) => [
    // Unique constraint: code unique per organization
    unique("unique_expense_category_code_per_org").on(
      table.organizationId,
      table.code
    ),
    // Unique constraint: name unique per organization
    unique("unique_expense_category_name_per_org").on(
      table.organizationId,
      table.name
    ),
    index("idx_expense_categories_org").on(table.organizationId),
...
export const expenses = org.table(
  "expenses",
  {
    ...
    exchangeRate: numeric("exchange_rate", { precision: 10, scale: 6 })
      .notNull()
      .default("1"),
    amountInBaseCurrency: numeric("amount_in_base_currency", { precision: 15, scale: 2 })
      .notNull(),
    ...
    // Receipt Information
    receiptStatus: receiptStatusEnum("receipt_status").notNull().default("required"),
    ...
    // Reimbursement
    isReimbursable: boolean("is_reimbursable").default(true),
    reimbursementAmount: numeric("reimbursement_amount", { precision: 15, scale: 2 })
      .notNull()
      .default("0"),
  ...
  (table) => [
    // Unique constraint: expenseNumber unique per organization
    unique("unique_expense_number_per_org").on(
      table.organizationId,
      table.expenseNumber
    ),
    ...
    index("idx_expenses_bid").on(table.bidId),
    ...
    index("idx_expenses_reimbursement_status").on(table.reimbursementStatus),
...
export const expenseReports = org.table(
  "expense_reports",
  {
    ...
    totalAmount: numeric("total_amount", { precision: 15, scale: 2 })
      .notNull()
      .default("0"),
    totalReimbursable: numeric("total_reimbursable", { precision: 15, scale: 2 })
      .notNull()
      .default("0"),
    totalNonReimbursable: numeric("total_non_reimbursable", { precision: 15, scale: 2 })
      .notNull()
      .default("0"),
    totalMileage: numeric("total_mileage", { precision: 10, scale: 2 })
      .notNull()
      .default("0"),
    totalMileageAmount: numeric("total_mileage_amount", { precision: 15, scale: 2 })
      .notNull()
      .default("0"),
...
export const expenseReportItems = org.table(
  ...
  (table) => [
    ...
    unique("unique_expense_per_report").on(
      table.reportId,
      table.expenseId
    ),
...
export const expenseReceipts = org.table(
  ...
  (table) => [
    index("idx_expense_receipts_org").on(table.organizationId),
...
export const expenseApprovals: any = org.table(
  "expense_approvals",
  {
    ...
    // Escalation
    escalatedFrom: uuid("escalated_from").references((): any => expenseApprovals.id),
  ...
  (table) => [
    index("idx_expense_approvals_org").on(table.organizationId),
...
export const expenseAllocations = org.table(
  "expense_allocations",
  {
    ...
    // Allocation Target
    allocationType: varchar("allocation_type", { length: 50 }).notNull(), // "job", "bid", "department", "general"
    jobId: uuid("job_id").references(() => jobs.id),
    bidId: uuid("bid_id").references(() => bidsTable.id),
  ...
  (table) => [
    index("idx_expense_allocations_org").on(table.organizationId),
    index("idx_expense_allocations_expense").on(table.expenseId),
    index("idx_expense_allocations_job").on(table.jobId),
    index("idx_expense_allocations_bid").on(table.bidId),
...
/**
 * Mileage Logs Table
 * Detailed mileage tracking for travel expenses
 */
export const mileageLogs = org.table(
  ...
  (table) => [
    index("idx_mileage_logs_org").on(table.organizationId),
    index("idx_mileage_logs_employee").on(table.employeeId),
    ...
    index("idx_mileage_logs_bid").on(table.bidId),
...
export const expenseReimbursements = org.table(
  ...
  {
    id: uuid("id").defaultRandom().primaryKey(),
    reimbursementNumber: varchar("reimbursement_number", { length: 100 }).notNull(),
...
export const expenseReimbursementItems = org.table(
  ...
  (table) => [
    ...
    index("idx_expense_reimbursement_items_reimbursement").on(table.reimbursementId),
...
export const expenseHistory = org.table(
  ...
  (table) => [
    index("idx_expense_history_org").on(table.organizationId),
