...
export const vehicles = org.table(
  "vehicles",
  {
    ...
    vehicleId: varchar("vehicle_id", { length: 50 }).notNull().unique(), // VEH-001, VAN-002, etc.
    ...
    serviceHistoryCostLast12Months: numeric("service_history_cost_last_12_months", {
      precision: 15,
      scale: 2,
    }).default("0"),
...
export const maintenanceRecords = org.table(
  "maintenance_records",
  {
    ...
    // Assignment
    assignedTo: varchar("assigned_to", { length: 255 }), // Display name
    assignedToEmployeeId: integer("assigned_to_employee_id").references(
      () => employees.id
    ),
  ...
  (table) => [
    ...
    index("idx_maintenance_overdue").on(
      table.status,
      table.scheduledDate
    ),
  ]
...
export const fuelRecords = org.table(
  "fuel_records",
  {
    ...
    costPerGallon: numeric("cost_per_gallon", { precision: 10, scale: 4 }).notNull(),
...
export const checkInOutRecords = org.table(
  "check_in_out_records",
  {
    ...
    // Driver Information
    driver: varchar("driver", { length: 255 }).notNull(), // Display name
    driverId: integer("driver_id").references(() => employees.id), // Employee ID
    driverAvatar: varchar("driver_avatar", { length: 500 }), // Avatar URL
    ...
    // Job Association
    jobId: uuid("job_id").references(() => jobs.id), // Link to job
  ...
  (table) => [
    ...
    index("idx_check_in_out_driver").on(table.driverId),
    index("idx_check_in_out_job").on(table.jobId),
...
export const assignmentHistory = org.table(
  "assignment_history",
  {
    ...
    // Metrics
    mileageDriven: numeric("mileage_driven", { precision: 10, scale: 2 }).default(
      "0"
    ),
    
    // Job Association
    jobId: uuid("job_id").references(() => jobs.id),
    jobType: varchar("job_type", { length: 100 }), // HVAC Installation, Maintenance, etc.
    jobLocation: varchar("job_location", { length: 255 }),
  ...
  (table) => [
    ...
    index("idx_assignment_history_driver").on(table.driverId),
    ...
    index("idx_assignment_history_active").on(
      table.vehicleId,
      table.status
    ),
  ]
...
export const vehicleMetrics = org.table(
  "vehicle_metrics",
  {
    ...
    // Performance Metrics
    utilizationRate: numeric("utilization_rate", { precision: 5, scale: 2 }).default(
      "0"
    ), // Percentage of days assigned
    costPerMile: numeric("cost_per_mile", { precision: 10, scale: 4 }).default("0"), // Total costs / miles driven
    ...
    totalFuelCost: numeric("total_fuel_cost", { precision: 15, scale: 2 }).default(
      "0"
    ),
