...
export const getComplianceCasesQuerySchema = z.object({
  query: z.object({
    ...
    sortBy: z
      .enum(["createdAt", "dueDate", "severity", "status"])
      .optional()
      .default("createdAt"),
...
export const createComplianceCaseSchema = z.object({
  body: z.object({
    ...
    disciplinaryAction: z
      .string()
      .max(100, "Disciplinary action is too long (maximum 100 characters)")
      .optional(),
    actionDate: z
      .string()
      .optional()
      .refine((val) => !val || !isNaN(Date.parse(val)), {
        message:
          "Invalid action date format. Please use YYYY-MM-DD format (e.g., 2024-12-15)",
      }),
    actionNotes: z.string().optional(),
    performanceImpact: z
      .number()
      .min(-10, "Performance impact cannot be less than -10")
      .max(0, "Performance impact cannot be greater than 0")
      .optional(),
...
export const getViolationWatchlistQuerySchema = z.object({
  query: z.object({
    ...
    minViolations: z
      ...
      .pipe(z.number().int().positive()),
    sortBy: z
      .enum(["violationCount", "employeeName", "department"])
      .optional()
      .default("violationCount"),
    sortOrder: z.enum(["asc", "desc"]).optional().default("desc"),
  }),
});
...
export const createEmployeeViolationSchema = z.object({
  body: z.object({
    ...
    complianceCaseId: z
      .string()
      .uuid("Compliance case ID must be a valid UUID")
      .optional(),
    violationType: z.enum(
      ["safety", "timesheet", "conduct", "training", "certification", "other"],
      {
        message:
    ...
    resolutionDate: z
      ...
      .refine((val) => !val || !isNaN(Date.parse(val)), {
        message:
          "Invalid resolution date format. Please use YYYY-MM-DD format (e.g., 2024-12-15)",
      }),
...
export const getViolationCountsQuerySchema = z.object({
  query: z.object({
    ...
    dateTo: z.string().optional(),
    groupBy: z
      .enum(["type", "severity", "employee", "department", "job"])
      .optional()
      .default("type"),
  }),
});
