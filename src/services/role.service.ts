...
export const assignRoleToUser = async (userId: string, roleId: number) => {
  try {
    ...
    if (existing.length > 0) {
      ...
      // Revoke all active sessions and clear the in-memory auth cache so the
      // new role permissions take effect immediately on the next request.
    ...
    // Revoke all active sessions and clear the in-memory auth cache so the
    // new role permissions take effect on the next login.
    await revokeAllUserSessions(userId);
    invalidateUserAuthCache(userId);
