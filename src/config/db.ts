...
const testNetworkConnectivity = (host: string, port: number, timeout: number = 5000): Promise<boolean> => {
...
const parseConnectionString = (connectionString: string): { host: string; port: number } | null => {
  try {
...
export const initDB = async (retries = 5, delay = 5000) => {
  ...
  if (connectionInfo) {
    console.log(`🌐 Testing network connectivity to ${connectionInfo.host}:${connectionInfo.port}...`);
    const isReachable = await testNetworkConnectivity(connectionInfo.host, connectionInfo.port, 10000);
    
    if (!isReachable) {
      console.error(`❌ Network connectivity test failed: Cannot reach ${connectionInfo.host}:${connectionInfo.port}`);
      console.error("💡 This indicates a network/firewall issue:");
      console.error("   - The database server may be blocking connections from this IP");
      console.error("   - Firewall rules may need to whitelist this server's IP address");
      console.error("   - The database host/port may be incorrect");
      console.error("   - Network routing issues between servers");
      console.error(`   - Check if ${connectionInfo.host}:${connectionInfo.port} is accessible from this location`);
      console.error("   - Compare network configuration with the working server (Germany)");
      process.exit(1);
    } else {
      console.log(`✅ Network connectivity test passed: ${connectionInfo.host}:${connectionInfo.port} is reachable`);
    }
  }
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (errorMessage.includes("timeout") || errorMessage.includes("ECONNREFUSED")) {
        ...
        if (connectionInfo) {
          console.error(`   - Verify firewall allows connections from this server to ${connectionInfo.host}:${connectionInfo.port}`);
          console.error("   - Check if database server whitelist includes this server's IP address");
        }
      ...
      if (i < retries - 1) {
        ...
        if (connectionInfo) {
          console.error(`💡 Network test passed, but PostgreSQL connection failed. Check:`);
